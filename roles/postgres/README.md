...
**Примечание:** В текущей реализации импорт выполняется при каждом запуске роли,
так как задача импорта не имеет условия `when`, проверяющего результат проверки.
SQL-скрипт использует `DROP TABLE IF EXISTS`, поэтому данные перезаписываются,
но это не является идемпотентным поведением.
