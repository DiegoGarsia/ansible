...
Роль является условно идемпотентной.
...
завершается без ошибок, но может переустанавливать Docker Compose плагин
(из-за `force: true` в `get_url`) и пересобирать Docker-образы
(из-за `build: always` в `docker_compose_v2`).

Это не влияет на работоспособность, но увеличивает время выполнения.
