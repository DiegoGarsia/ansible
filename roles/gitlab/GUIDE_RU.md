...
Роль выполняется на одной ВМ и в следующем порядке: открывает сетевые порты, устанавливает Docker, подключает официальный репозиторий GitLab, устанавливает GitLab CE, включает встроенный Container Registry, применяет оптимизацию для малой ВМ, ждёт готовности GitLab, устанавливает GitLab Runner и включает его службу `gitlab-runner.service`. Если задан token, Runner регистрируется как Docker Runner. В конце роль при необходимости создаёт или обновляет переменные конкретного проекта через GitLab API.

Оптимизация включена по умолчанию (`gitlab_resource_optimization: true`). Она отключает Prometheus, Alertmanager, Grafana, exporters и Kubernetes Agent, устанавливает два Puma worker и Sidekiq concurrency 10. При изменении этого блока handlers строго выполняют `gitlab-ctl reconfigure`, затем `gitlab-ctl restart`; после этого роль запускает `gitlab-ctl status`. Чтобы отказаться от этой настройки, установите `gitlab_resource_optimization: false` и повторите playbook.
